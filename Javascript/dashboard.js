...
import {addChildForm, removeChildForm, saveChildren as saveChildrenToFirestore} from "./children.js";
  ...
  // Apply Saved Preferences on Dashboard Load
  ...
  // Screen Wake Lock with NoSleep.js fallback for iOS
let wakeLock = null;
let noSleep = null;

async function requestWakeLock() {
  if ('wakeLock' in navigator) {
    // Use Wake Lock API for Android/Chrome
    try {
      wakeLock = await navigator.wakeLock.request('screen');
      console.log('Wake lock acquired via Wake Lock API');
      ...
      // Fall back to NoSleep.js if Wake Lock fails
      enableNoSleep();
    }

  } else {
    // Use NoSleep.js for iOS Safari and unsupported browsers
    enableNoSleep();
  }
}
...
function enableNoSleep() {
  ...
  try {
    noSleep = new NoSleep();
    // NoSleep requires a user gesture to activate on iOS
    // We attach it to the first user interaction on the page
    const enableOnInteraction = async () => {
      await noSleep.enable();
      console.log('NoSleep enabled via user interaction');
...
// Reacquire wake lock when user returns to tab
...
let currentRouteColor = "#354a5f"; // Light mode default
...
async function initializeDashboard(userId) {
  try {
    ...
    await applyUserPreferences(userId);
...
// Save Children wrapper for dashboard
async function saveChildrenFromDashboard() {
  const parentDoc = await getDoc(doc(db, 'users', auth.currentUser.uid));
  const assignedDriverId = parentDoc.data().assignedDriver;

  const success = await saveChildren(auth.currentUser.uid, assignedDriverId);
  if (success) {
...
// Route Dark Mode
function updateRouteTheme(isDark) {
    currentRouteColor = isDark
        ? "#00d4ff"      // Dark mode
        : "#354a5f";     // Light mode
    if (!routingControl) return;
    // Force the route to redraw
    routingControl.route();

    if (preferences.darkMode) {
      document.body.classList.add("dark-mode");
      updateMapTheme(true);
      updateRouteTheme(true);
    } else {
      updateMapTheme(false);
      updateRouteTheme(false);
}
}
...
window.saveChildren = saveChildrenFromDashboard;